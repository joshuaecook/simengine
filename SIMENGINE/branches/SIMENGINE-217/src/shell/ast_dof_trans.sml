...
local
	...
	let
	    val iterator = case settings of
				  ...
				  | NONE => NONE)
			     | NONE => NONE
	    (* look to see if one was specified as a 'dimension' *)
	    val iterator = case iterator of
			       SOME _ => iterator
			     | NONE => (case dimensions of 
					    NONE => NONE
					  | SOME [sym] => SOME (sym, Iterator.RELATIVE 0)
					  | SOME _ => (error ("unexpected multiple dimensions on output " ^ (Symbol.name name));
						       NONE))
	in
...
val ast_to_dof = Profile.time "AST to IR" ast_to_dof
