/*
 (require 'dsl-move)
 (require 'dsl-indent)
(defun my-play-with-parser-state ()
  (interactive)
  (set 'parser-state
    (syntax-ppss))
  (message "parser state %s" parser-state)
)

(syntax-ppss-toplevel-pos parser-state)
(syntax-ppss)

(dsl-parser-comment-style parser-state)

 (dsl-indent-parser-state-bo-comment parser-state)
*/
