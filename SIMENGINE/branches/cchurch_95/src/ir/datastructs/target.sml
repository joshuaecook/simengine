...
fun target2str t =
    case t 
     of CPU => "CPU"
      | OPENMP => "OPENMP"
      | CUDA {compute=COMPUTE11, multiprocessors, globalMemory} => "CUDA {compute capability: 1.1, # of multi-processors: "^(i2s multiprocessors)^", global memory (KB): "^(i2s globalMemory)^"}"
