...
for i=1:length(dsl_files)
    ...
    try
        ...
        error_list{length(error_list)+1} = struct('name',name,'me',me);
    end
end

if error_count > 0
    for i=1:length(error_list)
        disp(sprintf('Error encountered when running %s: ''%s'', %s', error_list{i}.name, error_list{i}.me.identifier, error_list{i}.me.message));
    end
