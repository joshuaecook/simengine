...
int parse_args(int argc, char **argv, simengine_opts *opts){
  ...
  while(1){
    ...
    case EMULATE:
      opts->emulate = 1;
      printf("Device emulation enabled\n");
      break;
    case DEBUG:
      opts->debug = 1;
...
int runsimEngine (const simengine_opts *opts)
{
  ...
  if(!opts->remake){
    ...
    snprintf(settings, BUFSIZE, "%s.template.settings = {target=\\\"%s\\\",precision=\\\"%s\\\",num_models=%d,emulate=%s,debug=%s,profile=%s}", opts->model_name, opts->target, opts->precision, opts->num_models, opts->emulate ? "true" : "false", opts->debug ? "true" : "false", opts->profile ? "true" : "false");
