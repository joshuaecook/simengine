...
structure Prof = MLton.Profile
fun profiled (f, d) x =
    Prof.withData (d, fn _ => f x)
fun profileDone (data, file) =
    Prof.Data.write (data, file) before Prof.Data.free data
...
(*			  handle e => (app (fn(s) => print("    " ^ s ^ "\n")) (MLton.Exn.history e))*)
	      ...
	      val forkedModels = 
		  let val forkModelProfile = Prof.Data.malloc ()
		      val forkModel = profiled (ModelProcess.forkModel, forkModelProfile)
		  in 
		      forkModel (CurrentModel.getCurrentModel()) before
