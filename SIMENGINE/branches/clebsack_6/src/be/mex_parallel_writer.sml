...
fun createExternalStructure props (class: DOF.class) = 
	...
	fun findStatesInitValues basestr (class:DOF.class) = 
		...
		fun exp2name exp = 
		    ...
		    handle e => DynException.checkpoint ("MexParallelWriter.createExternalStructure.findStatesInitValues.exp2name ["^(ExpProcess.exp2str exp)^"]") e
		...
		val sorted_init_conditions = 
			...
			(fn(exp)=>
			   ...
			   in
				   ...
				   SOME v => v
				 | NONE => DynException.stdException(("No initial condition found for differential equation: " ^ (ExpProcess.exp2str exp)), "MexParallelWriter.createExternalStructure.findStatesInitValues", Logger.INTERNAL)
			   end)
			diff_eqs_symbols
	    ...
	    handle e => DynException.checkpoint "MexParallelWriter.createExternalStructure.findStatesInitValues" e
    ...
    handle e => DynException.checkpoint "MexParallelWriter.createExternalStructure" e
...
fun buildMexHelp name = 
    ...
    in
	write_help(name ^ "_parallel.m", progs)
    end
...
fun buildMex (model: DOF.model as (classes, inst, props)) =
	...
	val header_progs = CParallelWriter.header (class_name, 
					   ["<mex.h>"],
...
(*								       outputdatastruct_progs @ 
    ...
    handle e => DynException.checkpoint "MexParallelWriter.buildMex" e
