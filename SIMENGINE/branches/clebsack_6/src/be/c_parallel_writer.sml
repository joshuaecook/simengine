...
fun main_code class =
	...
	[$("/* Transmutes the internal data buffer into the structured output"),
	 ...
	 SUB[$("CDATAFORMAT model_states[semeta.num_models*seint.num_states];"),
		 ...
		 SUB[$("seresult->outputs[modelid*semeta.num_models+outputid].alloc = START_SIZE;"),
		     $("seresult->outputs[modelid*semeta.num_models+outputid].num_quantities = seint.output_num_quantities[outputid];"),
		     $("seresult->outputs[modelid*semeta.num_models+outputid].num_samples = 0;"),
		     $("seresult->outputs[modelid*semeta.num_models+outputid].data = (se_alloc.malloc)(START_SIZE*seint.output_num_quantities[outputid]*sizeof(CDATAFORMAT));")],
		 $("}")],
	     $("}"),
	     $(""),
	     $("// Run the model"),
