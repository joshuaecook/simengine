...
__DEVICE__ int SOLVER(rk4, eval, TARGET, SIMENGINE_STORAGE, rk4_mem *mem, unsigned int modelid) {
  int i;
  int ret;
  ret = model_flows(mem->props->time[modelid], mem->props->model_states, mem->k1, mem->props->inputs, mem->props->outputs, mem->props->ob, 1, modelid);
  ...
  ret |= model_flows(mem->props->time[modelid]+(mem->props->timestep/2), mem->temp, mem->k2, mem->props->inputs, mem->props->outputs, mem->props->ob, 0, modelid);

  for(i=mem->props->statesize-1; i>=0; i--) {
    mem->temp[STATE_IDX] = mem->props->model_states[STATE_IDX] +
      (mem->props->timestep/2)*mem->k2[STATE_IDX];
  }
  ret |= model_flows(mem->props->time[modelid]+(mem->props->timestep/2), mem->temp, mem->k3, mem->props->inputs, mem->props->outputs, mem->props->ob, 0, modelid);

  for(i=mem->props->statesize-1; i>=0; i--) {
    mem->temp[STATE_IDX] = mem->props->model_states[STATE_IDX] +
      mem->props->timestep*mem->k3[STATE_IDX];
  }
  ret |= model_flows(mem->props->time[modelid]+mem->props->timestep, mem->temp, mem->k4, mem->props->inputs, mem->props->outputs, mem->props->ob, 0, modelid);
