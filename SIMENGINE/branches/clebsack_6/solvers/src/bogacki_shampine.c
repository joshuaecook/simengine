...
int bogacki_shampine_eval(bogacki_shampine_mem *mem, int modelid) {
  ...
  while(!appropriate_step) {
    ...
    ret |= model_flows(*(mem->props->time)+(cur_timestep[modelid]/2), mem->temp, mem->k2, mem->props->inputs, mem->props->outputs, 0, modelid);

    for(i=mem->props->statesize-1; i>=0; i--) {
      mem->temp[i*mem->props->num_models + modelid] = mem->props->model_states[i*mem->props->num_models + modelid] +
	(3*cur_timestep[modelid]/4)*mem->k2[i*mem->props->num_models + modelid];
    }
    ret |= model_flows(*(mem->props->time)+(3*cur_timestep[modelid]/4), mem->temp, mem->k3, mem->props->inputs, mem->props->outputs, 0, modelid);
    ...
    ret |= model_flows(*(mem->props->time)+cur_timestep[modelid], mem->next_states, mem->k4, mem->props->inputs, mem->props->outputs, 0, modelid);
