...
%
    ...
    if length(t1) > 0
      ...
      interface = rmfield(interface, {'hashcode', 'version', 'outputMode', 'outputPeriods', 'precision', 'parallel_models'});
      varargout = {interface};
    end
end
...
function cleanUp (options)
    if ~options.debug
        ...
        if ~status
            warning(['Could not remove temporary data directory: ' options.outputs '. ' ...
