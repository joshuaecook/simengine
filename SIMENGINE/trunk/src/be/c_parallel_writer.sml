	...
	fun init_props iter_sym =
		...
		fun progs () =
			...
			[(* HACK BEGIN *)
			 ...
			 $("props[ITERATOR_"^itername^"].count = (unsigned int*)calloc(PARALLEL_MODELS, sizeof(unsigned int));"),
			 $("props[ITERATOR_"^itername^"].dirty_states = (unsigned int*)calloc(PARALLEL_MODELS, sizeof(unsigned int));"),
			 $("props[ITERATOR_"^itername^"].ready_outputs = (unsigned int*)calloc(PARALLEL_MODELS, sizeof(unsigned int));"),
	...
	[
	 ...
	 [$("for(i=0;i<NUM_ITERATORS;i++){"),
	  SUB[$("Iterator iter = ITERATORS[i];"),
	      ...
	      $("if (props[iter].count) free(props[iter].count);"),
	      $("if (props[iter].dirty_states) free(props[iter].dirty_states);"),
	      $("if (props[iter].ready_outputs) free(props[iter].ready_outputs);"),
...
fun algebraic_wrapper kind shardedModel iterators =
	...
	val base_iterator =
	    case kind
	     of UPDATE => 
		(fn (_, DOF.UPDATE dep) => CurrentModel.itersym2iter dep | (iter_name,_) => DynException.stdException(("Unexpected iterator '"^(Symbol.name iter_name)^"'"), "CParallelWriter.algebraic_wrapper", Logger.INTERNAL))
	      | PREPROCESS =>
		(fn (_, DOF.ALGEBRAIC (DOF.PREPROCESS, dep)) => CurrentModel.itersym2iter dep | (iter_name,_) => DynException.stdException(("Unexpected iterator '"^(Symbol.name iter_name)^"'"), "CParallelWriter.algebraic_wrapper", Logger.INTERNAL))
	      | INPROCESS =>
		(fn (_, DOF.ALGEBRAIC (DOF.INPROCESS, dep)) => CurrentModel.itersym2iter dep | (iter_name,_) => DynException.stdException(("Unexpected iterator '"^(Symbol.name iter_name)^"'"), "CParallelWriter.algebraic_wrapper", Logger.INTERNAL))
	      | POSTPROCESS =>
		(fn (_, DOF.ALGEBRAIC (DOF.POSTPROCESS, dep)) => CurrentModel.itersym2iter dep | (iter_name,_) => DynException.stdException(("Unexpected iterator '"^(Symbol.name iter_name)^"'"), "CParallelWriter.algebraic_wrapper", Logger.INTERNAL))
	...
	fun iterator_call iter_sym =
		...
		CurrentModel.withModel
			...
			let
			    ...
			    val time = A.Variable (Symbol.name iter_name)
			    ...
			    val od = 
				A.Cast (A.Variable ("output_"^(Symbol.name iter_name)), T.C"CDATAFORMAT*")
			    val func_args =
				case base_iter_typ
				 of DOF.CONTINUOUS _ =>
...
val update_wrapper = algebraic_wrapper UPDATE
val preprocess_wrapper = algebraic_wrapper PREPROCESS
val inprocess_wrapper = algebraic_wrapper INPROCESS
val postprocess_wrapper = algebraic_wrapper POSTPROCESS
...
local 
    ...
    val v = Vector.fromList
in
fun equations_to_blocks label classname is_top_class (iter as (iter_sym, iter_type)) cc =
...
and instanceeq_to_block label (exp, is_top_class, iter as (iter_sym, iter_type), cc) =
    let
	...
	val inputs = ! (#inputs instclass)
	val iter_name = Symbol.name (case iter_type 
				      of DOF.UPDATE v => v
				       | _ => iter_sym)
	val iter_name' = Symbol.name (case iter_type 
				       of DOF.UPDATE v => v
...
and outputeq_to_blocks label (exp, is_top_class, iter as (iter_sym, iter_type), cc) =
    let
	val {classname, instname, props, inpargs, outargs} = ExpProcess.deconstructInst exp
	val instclass = CurrentModel.classname2class classname
	val iter_name = Symbol.name (case iter_type 
...
fun class_flow_code (class, is_top_class, iter as (iter_sym, iter_type)) =
	...
	val flow_function =
		...
		let
			...
			let val id = ref 0 in
			 fn () => ("spil_flow_"^(Symbol.name (#name class))^"_eq_"^(Int.toString (!id)) before id := 1+(!id))
			end
...
fun buildC (orig_name, shardedModel) =
	...
	val updateModels = (List.filter (fn {iter_sym, ...} => case itersym2iter iter_sym of (_, DOF.UPDATE _) => true | _ => false) shards,
			    sysprops)
	val updateIterators = ShardedModel.iterators updateModels
	...
	val iterator_wrappers_c = 
	    (update_wrapper updateModels updateIterators) @ 
