...
(* graphs of bounded size, they are dynamic within a bound specified
   by the argument to emptyBound *)

signature BOUNDED_GRAPH =
sig
  include GRAPH
  val emptyBound : int -> ('a,'b) graph
end

signature UNLAB_BOUNDED_GRAPH =
sig
  include UNLAB_GRAPH
  val emptyBound : int -> 'a graph
end



(* static graphs, they cannot grow 
   => graphs can only built by means of mkgr *)

signature STATIC_GRAPH = GRAPH
signature UNLAB_STATIC_GRAPH = UNLAB_GRAPH
