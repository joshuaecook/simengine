...
*/
model (Vm, Vmp, Raxonal, Rperiaxonal) = node(diameter, Istim, Iaxonal, Iperiaxonal)
  iterator t_explicit with {continuous, solver=forwardeuler{dt=0.001}}
  iterator t_implicit with {continuous, solver=cvode{abstol=1e-3}}
...
//State Declarations
//************************************************
state Vm = -80 with {iter=t_implicit}
state m  = 0.0732093 with {iter=t_explicit} //0.0679
state h  = 0.620695 with {iter=t_explicit} //0.6209
state p  = 0.202604 with {iter=t_explicit}
state s = 0.0430299 with {iter=t_explicit}
...
equations
   ...
   ICmem = Istim - IPas - Iaxonal - INaF - INaP -IK
